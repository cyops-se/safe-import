  ...
  export default {
    ...
    data: () => ({
      ...
      headers: [
        ...
        { text: 'Query', value: 'query', width: '40%' },
        { text: 'Match query', value: 'matchquery', width: '40%' },
      ...
      whiteheaders: [
        ...
        { text: 'Match query', value: 'matchquery', width: '40%' },
        { text: 'Count', value: 'count', width: '10%' },
        { text: 'Allowed', value: 'allowed', width: '120' },
        { text: 'Actions', value: 'actions', width: 1, sortable: false },
      ],
      items: [],
      greyitems: [],
    ...
    methods: {
      ...
      makeWhite (item) {
        var t = this
        item.class = 'white'
        ApiService.put('dns/' + item.ID, item)
          .then(response => {
            t.update()
            this.$notification.success('DNS entry moved to white list!')
          }).catch(response => {
            console.log('ERROR response: ' + response)
          })
      },
      ...
      makeBlack (item) {
        ...
        ApiService.put('dns/' + item.ID, item)
          .then(response => {
            t.update()
            this.$notification.success('DNS entry moved to black list!')
          }).catch(response => {
            console.log('ERROR response: ' + response)
          })
      },
      ...
      makeGrey (item) {
        ...
        ApiService.put('dns/' + item.ID, item)
          .then(response => {
            t.update()
            this.$notification.success('DNS entry moved to grey list!')
          }).catch(response => {
            console.log('ERROR response: ' + response)
          })
      },
      ...
      pruneEntries () {
        var t = this
        ApiService.get('dns/prune')
          .then(response => {
            t.update()
            this.$notification.success('DNS entries pruned!')
          }).catch(response => {
            console.log('ERROR response: ' + response)
          })
      },
      ...
      save () {
        ...
        if (this.editedIndex > -1) {
          ...
          ApiService.put('dns/' + this.editedIndex.ID, this.editedItem)
            .then(response => {
              this.$notification.success('DNS entry updated!')
            }).catch(response => {
              this.$notification.error('Failed to update DNS entry!' + response + ', ' + JSON.stringify(kalle))
            })
        }
        this.close()
      },
      ...
      saveItem (item) {
        ApiService.put('dns/' + item.ID, item)
          .then(response => {
            ...
            this.$notification.error('Failed to update DNS entry!')
          })
        this.close()
      },
    },
