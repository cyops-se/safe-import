...
func (svc *JobsService) getAll(payload string) (interface{}, error) {
	fmt.Println("JobsService: getAll()")
...
func (svc *JobsService) requestUrlWait(payload string) (interface{}, error) {
	...
	repo := &types.Repository{}
	if result := common.DB.First(&repo, request.RepositoryID); result.Error != nil {
		svc.LogGeneric("error", "Error while accessing the database: %#v", result.Error)
		return nil, result.Error
	}
	...
	// Get the URI
	localfile := path.Join(repo.OuterPath, u.Path)
	innerfile := path.Join(repo.InnerPath, u.Path)
	if u.Path == "/" {
		localfile = path.Join(localfile, "index.html")
		innerfile = path.Join(innerfile, "index.html")
	}
